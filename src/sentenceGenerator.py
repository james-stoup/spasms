...
def generateSentence(cur, noun, sentim):
    ...
    cur.execute(
        "SELECT word FROM common_words WHERE (sentiment > %s and sentiment < %s) and (type = 'adjective')",
        (minSentim, maxSentim),
    )
    numOfAdjectives = cur.rowcount
    adjectives = cur.fetchall()
    ...
    cur.execute(
        "SELECT word FROM common_words WHERE (sentiment > %s and sentiment < %s) and (type = 'adverb')",
        (minSentim, maxSentim),
    )
    numOfAdverbs = cur.rowcount
    adverbs = cur.fetchall()
    ...
    cur.execute(
        "SELECT word FROM common_words WHERE (sentiment > %s and sentiment < %s) and (type = 'verb')",
        (minSentim, maxSentim),
    )
    numOfVerbs = cur.rowcount
    verbs = cur.fetchall()
    ...
    for x in range(len(fragSplit)):
        if "<noun>" in fragSplit[x]:
            ...
            fragSplit[x] = fragSplit[x].replace(
                "<adjective>", adjectives[randAdjNum][0]
            )
